...
interface UseDuelOperationsOptions {
  ...
  queueOfflineOperation?: (type: 'saveDuel' | 'convertToCombo', data: unknown) => void;
}
...
export function useDuelOperations({
   ...
   */
  const getOperationKey = useCallback((type: string) => {
    return type;
  }, []);
   ...
   */
  const saveDuel = useCallback(
    async (duelStateToSave: DuelState) => {
   ...
   */
  const convertDuelToCombo = useCallback(
    async (duelStateToConvert: DuelState, title: string) => {
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Failed to convert duel to combo';
