...
import type { DuelState, DuelAction, DuelDeck, AvailableAction, DuelCardInstance, HistoryAction } from "@/types/duel.types";
...
export function useDuelState(options: UseDuelStateOptions = {}): UseDuelStateReturn {
   ...
   */
  const dispatchDuelAction = useCallback((action: DuelAction) => {
    try {
      if (!state) {
  ...
  }, [state, addHistoryEntry, addLogEntry, createError, logError, currentDuelState, canUndo, undo, onError]);

  /**
   * Gibt verfügbare Aktionen zurück
   */
  const availableActions = useCallback((cardInstanceId?: string): AvailableAction[] => {
    if (!state) return [];
   ...
   */
  const startDuel = useCallback((deck: DuelDeck) => {
    try {
      ...
      console.error("[DuelError] Failed to start duel - is Error instance:", error instanceof Error);
  ...
  const effectiveAvailableActions = useCallback((cardInstanceId?: string): AvailableAction[] => {
