...
export function ComboVersionHistory({
  comboId,
  onVersionRestored,
}: ComboVersionHistoryProps) {
    ...
    try {
      ...
      if (result.error) {
        ...
        addToast({
          ...
          description: t("combo.versionHistory.createdDescription") || "Die Version wurde erfolgreich erstellt",
        });
        await loadVersions();
      }
    } catch (error) {
    ...
    try {
      ...
      if (result.error) {
        ...
        addToast({
          ...
          description: t("combo.versionHistory.restoredDescription") || "Die Version wurde erfolgreich wiederhergestellt",
        });
        setOpen(false);
        onVersionRestored?.();
      }
  ...
  const handleDelete = async (version: number) => {
    if (
      !confirm(
        t("combo.versionHistory.deleteConfirm") ||
          "Möchtest du diese Version wirklich löschen?"
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (result.error) {
        ...
        addToast({
          ...
          description: t("combo.versionHistory.deletedDescription") || "Die Version wurde erfolgreich gelöscht",
        });
        await loadVersions();
      }
    } catch (error) {
          ...
          <ScrollArea className="h-[400px] pr-4">
            {isLoading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                {t("combo.versionHistory.noVersions") ||
