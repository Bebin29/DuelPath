...
export function ComboList() {
  ...
  const { data, error, isLoading, mutate } = useSWR<{
    ...
    `/api/combos${filterDeckId ? `?deckId=${filterDeckId}` : ""}`,
    async (url) => {
  ...
  async function handleDelete(comboId: string) {
    ...
    try {
      ...
      addToast({
        variant: "error",
        title: t("combo.errors.deleteFailed") || "Fehler beim Löschen",
        description: err instanceof Error ? err.message : t("combo.errors.deleteFailed") || "Fehler beim Löschen",
      });
    }
  }
        ...
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>{t("combo.createFirstCombo") || "Erstelle deine erste Kombo"}</CardDescription>
            </div>
          </div>
        </CardHeader>
      </Card>
