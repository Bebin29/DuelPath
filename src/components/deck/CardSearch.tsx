...
import { Card as UICard, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/components/ui/card";
...
export function CardSearch({ onCardSelect, showAddButton = true }: CardSearchProps) {
  ...
  const autocompleteCacheRef = useRef<Map<string, { results: string[]; timestamp: number }>>(new Map());
  const archetypeCacheRef = useRef<Map<string, { results: string[]; timestamp: number }>>(new Map());
  ...
  const { data: searchData, error: searchError, isLoading } = useSWR<CardListResult>(
    searchKey,
    fetcher,
    {
  ...
  const performAutocomplete = useCallback(async (query: string, signal?: AbortSignal, requestId?: number) => {
  ...
  const performArchetypeAutocomplete = useCallback(async (query: string, signal?: AbortSignal, requestId?: number) => {
    if (!query || query.length < 2) {
      setArchetypeSuggestions([]);
    ...
    try {
      const response = await fetch(
        `/api/cards?archetype=${encodeURIComponent(query)}&limit=5`,
        { 
          signal,
          headers: {
            'Cache-Control': 'public, max-age=300', // 5 Minuten Cache
  ...
  const performRaceAutocomplete = useCallback(async (query: string, signal?: AbortSignal, requestId?: number) => {
    ...
    try {
      const response = await fetch(
        `/api/cards?race=${encodeURIComponent(query)}&limit=5`,
        { 
          signal,
          headers: {
            'Cache-Control': 'public, max-age=300', // 5 Minuten Cache
  ...
  function handleFilterChange(key: keyof CardSearchFilter, value: string | string[] | number | undefined) {
    setFilters((prev) => ({
      ...prev,
      [key]: value || undefined,
    }));
  ...
  function handleAutocompleteKeyDown(e: React.KeyboardEvent, suggestions: string[], selectedIndex: number, onSelect: (value: string) => void) {
  ...
  function handleArchetypeKeyDown(e: React.KeyboardEvent) {
    if (archetypeSuggestions.length > 0) {
      switch (e.key) {
        case "ArrowDown":
          e.preventDefault();
          setSelectedArchetypeIndex((prev) => (prev < archetypeSuggestions.length - 1 ? prev + 1 : prev));
          break;
        case "ArrowUp":
          e.preventDefault();
  ...
  const activeFilters = useMemo(() => {
    const active: Array<{ key: keyof CardSearchFilter; label: string; value: string; removeValue?: string }> = [];
    if (filters.type) active.push({ key: "type", label: t("deck.cardType"), value: filters.type });
    if (filters.attribute) active.push({ key: "attribute", label: t("deck.attribute"), value: filters.attribute });
    if (filters.level !== undefined) active.push({ key: "level", label: t("deck.level"), value: `Level ${filters.level}` });
    ...
    <div className="space-y-4">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <Input
              ...
              onKeyDown={(e) => handleAutocompleteKeyDown(e, autocompleteSuggestions, selectedAutocompleteIndex, handleAutocompleteSelect)}
      ...
      <div className="space-y-3">
        ...
        {activeFilters.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {activeFilters.map((filter, index) => (
              <Badge key={`${filter.key}-${index}-${filter.value}`} variant="secondary" className="gap-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="space-y-2">
            <Label className="text-xs" htmlFor="filter-type">{t("deck.cardType")}</Label>
            <Select
              value={filters.type || ""}
          ...
          <div className="space-y-2">
            <Label className="text-xs" htmlFor="filter-attribute">{t("deck.attribute")}</Label>
            <Select
          ...
          <div className="space-y-2">
            <Label className="text-xs" htmlFor="filter-level">{t("deck.level")}</Label>
            <Select
          ...
          <div className="space-y-2">
            <Label className="text-xs" htmlFor="filter-atk-min">{t("deck.atk")} (Range)</Label>
          ...
          <div className="space-y-2">
            <Label className="text-xs" htmlFor="filter-def-min">{t("deck.def")} (Range)</Label>
            <div className="flex gap-2 items-center">
              <Input
                id="filter-def-min"
                type="number"
          ...
          <div className="space-y-2">
            <Label className="text-xs" htmlFor="filter-race">{t("deck.race")}</Label>
          ...
          <div className="space-y-2 md:col-span-2">
            <Label className="text-xs" htmlFor="filter-archetype">{t("deck.archetype")}</Label>
            <div className="relative">
              <Input
                id="filter-archetype"
      ...
      {hasActiveSearch && (
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <Label htmlFor="sort-by" className="text-sm">{t("deck.sortBy")}</Label>
            <Select
              value={`${sortOptions.sortBy}-${sortOptions.order}`}
              onValueChange={(value) => {
                const [sortBy, order] = value.split("-") as [CardSortOptions["sortBy"], "asc" | "desc"];
      ...
      {/* Suchergebnisse */}
      <div className="space-y-2" role="region" aria-label={t("deck.searchResultsCount", { count: total })} aria-live="polite" aria-atomic="false">
        {error ? (
