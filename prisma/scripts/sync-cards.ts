  ...
  try {
    ...
    if (missingCards.length > 0) {
      ...
      for (let i = 0; i < missingCards.length; i += batchSize) {
        ...
        process.stdout.write(
          `\rProgress: ${i + batch.length}/${missingCards.length} (${percentage}%)`
        );
      }
      console.log('\n');
    } else {
    ...
    if (cardsNeedingUpdate.length > 0) {
      ...
      for (let i = 0; i < cardsNeedingUpdate.length; i += batchSize) {
        ...
        process.stdout.write(
          `\rProgress: ${Math.min(i + batchSize, cardsNeedingUpdate.length)}/${cardsNeedingUpdate.length}`
        );
      }
    ...
    console.log(
      `Cards with normalized fields updated: ${cardsNeedingUpdate.length > 0 ? cardsNeedingUpdate.length : 0}`
    );
    console.log('='.repeat(50));
